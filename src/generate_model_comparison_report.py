...
This script generates a comprehensive report showing the comparison
...
class ModelComparisonReport:
    ...
    def __init__(
        ...
        self.pdf.add_font('DejaVu', 'B', './fonts/DejaVuSansCondensed-Bold.ttf', uni=True)
    ...
    def create_executive_summary(self):
        ...
        data = [
            ["RMSE", f"{self.model1_metrics['rmse'].mean():.4f}", f"{self.model2_metrics['rmse'].mean():.4f}", f"{rmse_imp:.1f}%"],
            ["R²", f"{self.model1_metrics['r2'].mean():.4f}", f"{self.model2_metrics['r2'].mean():.4f}", f"{r2_imp:.1f}%"],
    ...
    def create_performance_comparison(self):
        ...
        if self.comparison_data:
            ...
            model1_rmse = self.model1_metrics['rmse'].mean() if not self.model1_metrics.empty else 0
            model2_rmse = self.model2_metrics['rmse'].mean() if not self.model2_metrics.empty else 0
            
            model1_mae = self.model1_metrics['mae'].mean() if not self.model1_metrics.empty else 0
            model2_mae = self.model2_metrics['mae'].mean() if not self.model2_metrics.empty else 0
            
            model1_r2 = self.model1_metrics['r2'].mean() if not self.model1_metrics.empty else 0
            model2_r2 = self.model2_metrics['r2'].mean() if not self.model2_metrics.empty else 0
            
            data = [
                ["RMSE", f"{model1_rmse:.4f}", f"{model2_rmse:.4f}", f"{perf_imp.get('rmse', 0):.1f}%"],
                ["MAE", f"{model1_mae:.4f}", f"{model2_mae:.4f}", f"{perf_imp.get('mae', 0):.1f}%"],
                ["R²", f"{model1_r2:.4f}", f"{model2_r2:.4f}", f"{perf_imp.get('r2', 0):.1f}%"]
            ]
...
def main():
    """Main function to generate both PDF and Markdown reports."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Generate model comparison reports')
    parser.add_argument('--data-dir', default='data/processed/model_comparison',
    ...
    if args.format in ['pdf', 'both']:
        try:
            report = ModelComparisonReport(args.data_dir, args.output_dir)
    ...
    if args.format in ['markdown', 'both']:
        try:
            md_path = create_markdown_report(args.data_dir, args.output_dir)
